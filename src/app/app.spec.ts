...
describe('App', () => {
  ...
  beforeEach(async () => {
    const yaftSpy = jasmine.createSpyObj('YaftProviderService', [
      'connect', 'loadFeatures', 'createFeature', 'updateFeature', 'deleteFeature',
      'getFeatureStatus', 'setCollectionSecret', 'getCurrentConnection', 'getCurrentFeatures'
    ], {
    ...
    const exportSpy = jasmine.createSpyObj('ExportService', [
      'exportToJson', 'exportToCsv', 'exportBooleanJson', 'exportBooleanCsv', 'importFromFile'
    ]);

    const errorSpy = jasmine.createSpyObj('ErrorHandlerService', [
      'showSuccessNotification', 'showErrorNotification', 'showInfoNotification', 'showWarningNotification'
    ]);

    const bulkSpy = jasmine.createSpyObj('BulkOperationsService', [
      'getOperableFeatures', 'enableFeatures', 'disableFeatures', 'deleteFeatures', 'exportFeatures'
    ]);
    ...
    mockYaftService = TestBed.inject(YaftProviderService) as jasmine.SpyObj<YaftProviderService>;
    ...
    mockBulkOperations = TestBed.inject(BulkOperationsService) as jasmine.SpyObj<BulkOperationsService>;
  });
  ...
  describe('Component Initialization', () => {
    ...
    it('should create connection and feature forms', () => {
      ...
      expect(component.connectionForm.get('providerType')?.value).toBe(ProviderType.API_SERVICE);
    });
  });

  describe('Provider Type Management', () => {
    it('should identify API provider types correctly', () => {
      component.connectionForm.patchValue({ providerType: ProviderType.API_SERVICE });
      expect(component.isApiProvider).toBe(true);

      component.connectionForm.patchValue({ providerType: ProviderType.LOCAL_STORAGE });
      expect(component.isApiProvider).toBe(false);
    });

    it('should identify boolean provider types correctly', () => {
      component.connectionStatus = { type: ProviderType.API_SERVICE_BOOLEAN, isConnected: true };
      expect(component.isBooleanProvider).toBe(true);

      component.connectionStatus = { type: ProviderType.API_SERVICE, isConnected: true };
      expect(component.isBooleanProvider).toBe(false);
    });

    it('should adjust displayed columns based on provider type', () => {
      component.connectionStatus = { type: ProviderType.API_SERVICE_BOOLEAN, isConnected: true };
      expect(component.currentDisplayedColumns).toEqual(['select', 'key', 'value', 'actions']);

      component.connectionStatus = { type: ProviderType.API_SERVICE, isConnected: true };
      expect(component.currentDisplayedColumns).toEqual(component.displayedColumns);
    });
  });
  ...
  describe('Connection Management', () => {
    ...
    it('should handle connection failure', () => {
      ...
      mockYaftService.connect.and.returnValue(throwError(() => new Error(errorMsg)));
    ...
    it('should set collection secret when provided', () => {
      ...
      expect(mockYaftService.setCollectionSecret).toHaveBeenCalledWith('test-secret');
    });
  ...
  describe('Feature Management', () => {
    ...
    it('should handle feature creation failure', () => {
      mockYaftService.createFeature.and.returnValue(throwError(() => new Error('Creation failed')));
    ...
    it('should not toggle feature without secret', () => {
      ...
      expect(component.alertMessage).toContain('Cannot toggle feature without secret');
    });
    ...
    it('should delete feature with confirmation', () => {
      ...
      expect(mockYaftService.deleteFeature).toHaveBeenCalledWith(feature.key, feature.secret);
    });
  ...
  describe('Tag Management', () => {
    ...
    it('should reject invalid tag format', () => {
      ...
      expect(component.alertMessage).toContain('Tags must be lowercase, alphanumeric, and hyphens only');
      expect(mockEvent.input.value).toBe('');
    });
  ...
  describe('Export/Import Operations', () => {
    ...
    it('should handle file import', () => {
      ...
      const mockResult = { success: true, features: [], warnings: [], errors: [] };
      
      mockExportService.importFromFile.and.returnValue(Promise.resolve(mockResult));
      spyOn(window, 'confirm').and.returnValue(true);
  ...
  describe('Utility Methods', () => {
    it('should copy text to clipboard', () => {
      ...
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith(feature.secret);
    });

    it('should check if feature has secret', () => {
      expect(component.hasSecret(mockFeatures[0])).toBe(true);
      expect(component.hasSecret({ key: 'no-secret', value: 'true' })).toBe(false);
    });

    it('should get feature status from service', () => {
      const mockStatus = { key: 'test', isEnabled: true, status: 'active' as const };
      mockYaftService.getFeatureStatus.and.returnValue(mockStatus);

      const status = component.getFeatureStatus(mockFeatures[0]);

      expect(mockYaftService.getFeatureStatus).toHaveBeenCalledWith(mockFeatures[0]);
