...
describe('YaFT Admin Integration Tests', () => {
  ...
  describe('API Provider Integration', () => {
    it('should complete full API workflow: connect, create, update, delete', (done) => {
      ...
      yaftService.connect(connection).subscribe(success => {
        ...
        yaftService.createFeature(newFeature).subscribe(createdFeature => {
          ...
          yaftService.updateFeature(createdFeature.key, { value: 'false' }, createdFeature.secret)
            .subscribe(updatedFeature => {
              expect(updatedFeature.value).toBe('false');

              // Step 4: Delete feature
              yaftService.deleteFeature(createdFeature.key, createdFeature.secret!)
                .subscribe(() => {
                  done();
                });

              // Mock delete request
              const deleteReq = httpMock.expectOne(`http://localhost:8080/features/${createdFeature.key}/${createdFeature.secret}`);
              deleteReq.flush({});
            });

          // Mock update request (deactivate)
          const updateReq = httpMock.expectOne(`http://localhost:8080/features/deactivate/${createdFeature.key}/${createdFeature.secret}`);
    ...
    it('should handle existing collection connection and feature loading', (done) => {
      ...
      const existingFeatures: Feature[] = [
        ...
        { key: 'existing-uuid|feature2', value: 'false', activeAt: '2024-01-01T00:00:00Z' }
      ];
  ...
  describe('Local Storage Provider Integration', () => {
    it('should complete full local storage workflow', (done) => {
      ...
      yaftService.connect(connection).subscribe(success => {
        ...
        yaftService.createFeature(newFeature).subscribe(createdFeature => {
          ...
          // Update feature
          yaftService.updateFeature('local-test', { value: 'false' }).subscribe(updatedFeature => {
            expect(updatedFeature.value).toBe('false');
            ...
            expect(features.find((f: any) => f.key === 'local-test').value).toBe('false');

            // Delete feature
            yaftService.deleteFeature('local-test').subscribe(() => {
              // Verify feature removed from localStorage
              const storedAfterDelete = localStorage.getItem('yaft-admin-features');
              const featuresAfterDelete = JSON.parse(storedAfterDelete!);
              expect(featuresAfterDelete.find((f: any) => f.key === 'local-test')).toBeUndefined();
              done();
            });
          });
        });
    ...
    it('should load existing features from localStorage', (done) => {
      ...
      localStorage.setItem('yaft-admin-features', JSON.stringify(existingFeatures));
  ...
  describe('Provider Type Switching Integration', () => {
    it('should switch from API to Local Storage provider', (done) => {
      ...
      apiReq.flush({ key: 'test-uuid|connection-test', value: 'true', secret: 'secret' });
  ...
  describe('Error Handling Integration', () => {
    it('should handle API connection failures gracefully', (done) => {
      ...
      yaftService.connect(connection).subscribe({
        ...
        error: (error) => {
          expect(error.message).toContain('Collection with UUID \'nonexistent\' not found');
          done();
        }
      });

      const req = httpMock.expectOne('http://localhost:8080/features/nonexistent');
